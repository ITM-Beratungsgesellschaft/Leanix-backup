...
class LeanIX:
    ...
    def __init__(self, apitoken, workspaceID, hostname):
    ...
    def take_snapshot(self):
        ...
        while status != "COMPLETED":
            ...
            while connection_tries < 3:
                try:
                    ...
                    print("Connection error, tried to connect: " + str(connection_tries + 1) + " times")
        ...
        download_url = self.instance + self.base_path + "/exports" + "/downloads/" + self.workspaceID
        ...
        #write to file
        filename = "backup/" + self.config['OPTIONAL']['EXPORT_FILENAME'].replace("{cdate}", self.get_today_date())
    ...
    def download_surveys(self):
        ...
        for survey in [x["id"] for x in survey_ids]:
            ...
            for run in [y['id'] for y in runs]:
                ...
                filename = "backup/" + self.config['OPTIONAL']['SURVEY_FILENAME'].replace("{cdate}", self.get_today_date()).replace("{id}", survey).replace("{run}", run)
...
if __name__ == '__main__':
    ...
    leanIX = LeanIX(apitoken, workspaceID, hostname)
    leanIX.connect()
